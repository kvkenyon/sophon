...
- Commander runtime policy lives in `internal/commander`; `pintellect home` creates one project-scoped session that binds to a mission after conversational intake, sessions reuse the M5 Herdr profiles and native resume identity, event-wake cursors advance only after Herdr accepts the structured mission message, and prompt sets resolve from the registered project before the binary install directory, never caller CWD.
...
- Startup recovery is coordinated in `internal/recovery`; the opt-in process-kill matrix is `test/crash-injection.sh`, with its real Herdr restart proof gated behind `PI_CRASH_HERDR_LAB=1` and the FirstMate lab helper.
