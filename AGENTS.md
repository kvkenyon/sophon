...
- Commander runtime policy lives in `internal/commander`; sessions reuse the M5 Herdr profiles, persist mission and native resume identity, advance their durable event-wake cursor only after Herdr accepts the structured mission message, and resolve prompt sets from the registered project before the binary install directory, never caller CWD.
