...
2. Find the most recent real operator-authored message before the current `/recap` invocation.
   ...
   Platform, system, developer, tool, and other injected operational messages are not operator messages merely because a transcript renders them with a user-like role.
   Do not exclude an ordinary operator message merely because it quotes operational content, and do not invent prefix matching to classify messages.
...
5. Inspect the whole visible session before the invocation for explicit operator decisions that are still visibly unanswered, including choices raised before the recap boundary.
   ...
   Include each open decision once and deduplicate by substance.
6. Do not run `sophon status`, read the data home, query GitHub, or use any other external source in the normal recap branch.
